...
function reg() {
    ...
    while(data_time_line < 180){
        ...
        switch(UI){
            ...
            break;
            case "com.maihan.tredian.activity.LoginActivity":
                (jsclick("text","微信一键登录",true,4));
            break;
            case "com.maihan.tredian.activity.VerifyPhoneActivity":

                if (jsclick("text","输入你的手机号",false,2)){
                ...
                }else if(jsclick("text","确定",true,5)){
                }
            break;
            case "com.maihan.tredian.activity.MainActivity":
                if(jsclick("text","点击登录",true,2)){
                }else if (jsclick("text","我的",true,2)){
                    if (jsclick("text","兑换提现",false,2)){
                        var d = className("TextView").id("user_today_coin_tv").findOne(2000);
                        if(d){
                            info["gold"] = d.text();
                            log(d.text())
                        }
                        var d = className("TextView").id("user_current_rmb_tv").findOne(2000);
                        if(d){
                            info["money"] = d.text();
                            log(d.text())
                        }
                        var d = className("TextView").id("user_invite_tv").findOne(2000);
                        if(d){
                            info["gift"] = d.text();
                            log(d.text())
                            log("注册成功");
            ...
            break;
            case "com.maihan.tredian.activity.WelcomeActivity":
...
function read(){
    ...
    while(data_time_line < 40){
        ...
        switch(UI){
            case "com.maihan.tredian.activity.NewsDetailActivity":
                if(look_times < look_timesKey && jsclick("text","我来说两句",false,2)){
            ...
            break;
            case "com.maihan.tredian.activity.MainActivity":
                if( jsclick("text","我的",false,2) && jsclick("text","热点",false,2) && jsclick("text","刷新",false,5)){
                    ...
                    sleep(1000*3)
                }else if(jsclick("text","热点",true,5)){
...
var apk_url = "img.wenfree.cn/apk/com.maihan.tredian.apk"
var app_name = "淘最热点";
var app_bid = "com.maihan.tredian"
var info={};
