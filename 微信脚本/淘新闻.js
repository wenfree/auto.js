...
function reg() {
    ...
    while(data_time_line < 180){
        ...
        switch(UI){
            ...
            break;
            case "com.coohua.xinwenzhuan.controller.MainActivity":
                if(textMatches("/写评论.*/").findOne(500)){
                    Back();
                }else if (jsclick("text","我的钱包",true,2)){
                    if(jsclick("text","微信一键登录",true,3)){
                    }else if (jsclick("text","登录/注册",true,3)){
                    }else if (jsclick("text","金币",false,2)){
                        var d = className("TextView").id("home_my_wallet_gold").findOne(2000);
                        if(d){
                            info["gold"] = d.text();
                            log(d.text())
                        }
                        var d = className("TextView").id("home_my_wallet_income").findOne(2000);
                        if(d){
                            info["money"] = d.text();
                            log(d.text())
                            log("注册成功");
                ...
                }else if(jsclick("text","微信一键登录",true,3)){
                }else{
                    Back();
                    tips_times++;
                    if (tips_times > 10){
...
function read(){
    ...
    while(data_time_line < 40){
        ...
        switch(UI){
            case "com.coohua.xinwenzhuan.controller.MainActivity":
                if( check_look && textMatches("/写评论.*/").findOne(500) ){
                ...
                }else if(textMatches("/写评论.*/").findOne(500)){
                    Back();
                }else if(look_times < look_timesKey){
                    Back();
                }else if( jsclick("text","我的钱包",false,2) && jsclick("text","推荐",false,2) && jsclick("text","新闻",false,5)){
                    jsclick("text","新闻",true,5)
                    ...
                    sleep(1000*3)
                }else if(jsclick("text","新闻",true,5)){
...
var apk_url = "img.wenfree.cn/apk/com.coohua.xinwenzhuan.apk"
var app_name = "淘新闻";
var app_bid = "com.coohua.xinwenzhuan"
var info={};
